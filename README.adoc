...
Although {apache-geode-name} caching can be embedded in (嵌入) web apps and WAR files, the simpler approach (方法) demonstrated (展示了) below (以下)
creates a standalone application. You package everything in a single, executable JAR file, driven (被驱动) by a good old Java
`main()` method.
